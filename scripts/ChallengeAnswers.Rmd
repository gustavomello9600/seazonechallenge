...
revenue.forecast.plot <- monthly.comissions.forecast %>%
  autoplot(monthly.comissions, level=80) +
  scale_y_log10() +
...
revenue.by.year <- monthly.comissions.actual.and.forecasted %>%
  as_tibble() %>%
...
p <- ggplotly(monthly.comissions.actual.and.forecasted %>%
...
# p <- ggplotly(revenue.forecast.plot, width=1200, height=600)
# api_create(p, filename = "r-area-plot-percentage-by-advance",
#            fileopt="overwrite", sharing="public")
